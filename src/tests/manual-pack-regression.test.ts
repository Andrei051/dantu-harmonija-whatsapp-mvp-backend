...
describe("manual pack regression (POST /messages/test)", () => {
  ...
  it("Ar galiu uzsirasyti balinimui -> booking_request + lt", async () => {
    const res = await request(app)
      .post("/messages/test")
      .send({ message: "Ar galiu uzsirasyti balinimui" });
  ...
  it("Kur yra klinika? -> clinic_location + lt", async () => {
    const res = await request(app)
      .post("/messages/test")
      .send({ message: "Kur yra klinika?" });
